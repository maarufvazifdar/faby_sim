...
def calc_distance(goals, x_current, y_current):
    ...
    for i in range(len(goals)):
        distance.append(math.sqrt((goals[i][0] - x_current)**2 + (goals[i][1] - y_current)**2))
...
def pose_callback(msg):
   ...
   robot_quat = [msg.pose.pose.orientation.x,msg.pose.pose.orientation.y,msg.pose.pose.orientation.z,msg.pose.pose.orientation.w]
...
if __name__ == '__main__':
        try:
            ...
            # x, y, yaw = float(argv[1]), float(argv[2]), float(argv[3])
            # result = nav_goal_py(x, y, yaw)
            # if result:
